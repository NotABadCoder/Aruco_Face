...
def crop_display(image_arr):
  ...
  if display_contour is None:
      print("No valid display contour found")
      return None
  else:
...
def normalize_contrs(img,cntr_pts):
  if cntr_pts is None:
    print("No valid contour points found")
    return None
...
def process_image(orig_image_arr):
  ...
  if display_image_arr is None:
    print("Display image is None")
    return None
...
def ocr_image(input):
  if isinstance(input, str):  # means the input is a filepath
    orig_image_arr = cv2.imread(input)
  else:  # means the input is a numpy array (image)
    orig_image_arr = input

  processed_image = process_image(orig_image_arr)
  if processed_image is None:
    print("Processed image is None")
    return None
  else:
    otsu_thresh_image = PIL.Image.fromarray(processed_image)
